...
- [ ] T024 [P] [US2] **[DEFERRED]** Add a creation-script field to `QuickInfoResult` (FR-017). The Understand workflow's contracts agent found FR-017's CREATE script belongs to the separate object-definition Script tab (`ObjectDefinitionPanel` + existing `ObjectDefinitionService`), NOT the hover — so it pairs with T027, not T025. No QuickInfoResponse field needed for the hover. Bundled into the T027 follow-up.
...
- [ ] T027 [US2] Populate the object-definition Script tab with the real CREATE script via `ObjectDefinitionService` (FR-017) — pairs with the deferred T024. (Separate `ObjectDefinitionPanel` surface, not the hover.)
