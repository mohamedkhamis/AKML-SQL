...
- [~] T010 [US1] (T007 unblocked) Cover DDL (CREATE TABLE/PROC alignment) + DML statement layout; idempotent + golden-clean. **(a) multi-char operator splitting — ✅ FIXED:** `>=`/`<=`/`<>`/`!=`/`!<`/`!>` tokenise as two adjacent single-char operator tokens and `LayoutEngine`'s operator-spacing split them (`>=`→`> =`); added `IsCompoundOperatorSecondHalf` + source-adjacency check to suppress the interior space. +`CompoundOperatorSpacingTests` (6 ops × join/keep-spaces/idempotent). Re-blessed 02/04/10/12/13 (operator-join only). **(b) JOIN-keyword splitting — ⏳ DEFERRED:** the base `LineBreakDecider` fix (break before the modifier INNER/LEFT, keep JOIN on that line; add `JoinOn` so chained joins break) makes rules-OFF correct, but rules-ON then *over-collapses* the FROM+JOIN region onto one line — a base+collapse interaction (the same collapse follow-up below). Reverted the base fix pending that; JOIN stays as the b79518e drift-guard golden. **(c)** DDL/PROC column+param alignment otherwise unverified.
