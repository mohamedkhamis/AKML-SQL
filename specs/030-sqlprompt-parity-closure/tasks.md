...
- [ ] T009 [US1] (T007 unblocked) Cover CASE/CTE/BEGIN-END/IF/TRY-CATCH layout; verify the procedural constructs the spike never exercised; confirm CASE-END-inside-BEGIN pairing. **Concrete bugs now observable** (the Stage-6 validator fix made 03/04/11/12/13 format for the first time — see research.md "Stage-6 validation gap"; repro in `StageSixValidationProbeTests`): **BEGIN/END block cramming** (all body statements collapse onto one line after `BEGIN` — 11-stored-procedure); **CTE-body-close / statement-boundary merge** (`…region) SELECT …` — 03/04; ORDER BY merged after a proc's `BETWEEN…AND` — 11); **MERGE `WHEN` clause layout** (12). Present rules-OFF too → base-pipeline (`LayoutEngine`/`TextEmitter`), not the rule sets.
- [ ] T010 [US1] (T007 unblocked) Cover DDL (CREATE TABLE/PROC alignment) + DML statement layout; idempotent + golden-clean. **Concrete bugs now observable** (02/10 newly formatting): **multi-char operator splitting** `>=`→`> =`, `<>`→`< >` (near-correctness — parses but alarming; base-pipeline spacing/`TextEmitter`); **JOIN-keyword splitting** `INNER JOIN`→`INNER`⏎`JOIN`, `LEFT JOIN` likewise (02/11). The 7 newly-formatting corpora (02/03/04/10/11/12/13) are **blessed as drift-guard goldens + Web baselines** capturing current (buggy) output — fixing the above will re-drift them.
