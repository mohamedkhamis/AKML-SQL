...
- **Formatter layout rules — investigated → NO-GO as-is** (P1 layout fidelity, FR-001): a de-risk spike + an 8-agent rollout-readiness workflow + three empirical checks proved the dormant `Rules/*` **cannot be enabled as-is** — they write systemic *absolute* indent (Dml/Ddl entirely hardcoded) and clobber LayoutEngine's nested indent, passing Stage-6/Stage-7 yet **regressing 36/610 human-blessed goldens**. The "built but not wired = cheap" thesis is **refuted** for the formatter. Layout fidelity needs an architectural decision (rework the rules' indent model · move layout into `LayoutEngine`/`LineBreakDecider` · or narrow scope) — a focused design effort, gated at task **T007**. See research **R1 (revised)**. The independent, genuinely-cheap wins (format-action dispatch + UX) remain unblocked.
