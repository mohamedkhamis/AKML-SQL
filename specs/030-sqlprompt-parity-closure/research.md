...
**T010 progress (2026-06-09): (a) operator-split FIXED.** `>=`/`<=`/`<>`/`!=`/`!<`/`!>` all tokenise as two adjacent single-char operator tokens (verified — `StageSixValidationProbeTests.Dump_Operator_Tokenization`); `LayoutEngine` operator-spacing split them. Fix: `IsCompoundOperatorSecondHalf` + a source-adjacency guard (`t.Offset == prevSemanticTokenEndOffset`) suppress the interior space, so `x >= y` / `x <> y` stay joined (user-written `x > =` is left alone). `CompoundOperatorSpacingTests` locks all six ops + single-operator-spacing + idempotency; re-blessed 02/04/10/12/13 (operator-join drift only). **(b) JOIN-split: base fix found but deferred.** A `LineBreakDecider` change (break before the join *modifier* INNER/LEFT; keep `JOIN` on that line via `prevSemanticTokenType`; add `JoinOn` to `IsJoinModifier` so chained joins break) makes rules-OFF JOIN layout correct — but rules-ON then over-collapses the whole FROM+JOIN region onto one line (a base↔collapse interaction, same family as the compact collapse-vs-expand follow-up). Reverted pending the collapse work; JOIN remains the drift-guard golden. The collapse-precedence rework is the gating piece for JOIN/MERGE/proc layout.
