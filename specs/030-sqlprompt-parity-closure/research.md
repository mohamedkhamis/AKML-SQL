...
**DmlRules — VERIFIED CORRECT (2026-06-08).** Regenerated the goldens with DmlRules-only on (`AKML_UPDATE_PARITY_GOLDEN=1`) and triaged the full `git diff`: all **13** changed goldens are legitimate **re-bless**, zero remaining bugs. (a) `09-between-operators` (×6 styles): BETWEEN's ANDs stay at their indent (fix works), clause-level ANDs align to WHERE (col 0), nested OR delta-shifts col 8→4 — all correct `alignWithWhere`. (b) `07-in-list-short` (×6): the clause AND after the IN-list aligns to WHERE — correct. (c) `01-simple-select__compact` (×1): the short statement collapses to one line — correct, because compact's `collapseThreshold` is 120 and the line is ~98 chars (default/others keep it multi-line at 80). **DmlRules indent + collapse are correct on the whole corpus; the only remaining DmlRules step is the final golden re-bless, deferred to the all-groups enable.** Goldens + the temp flip were reverted (final re-bless happens once, with all rules on). **Next group: DdlRules** (same flip→triage→fix cycle).
