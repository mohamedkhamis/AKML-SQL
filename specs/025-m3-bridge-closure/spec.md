...
3. **In-flight WebSocket revocation** (spec 021 T066-partial) — drop open sockets the moment `BearerTokenStore.RevokeByHash` runs; the next handshake already rejects revoked bearers, so the gap is only the in-flight grace window. **Investigation 2026-05-28**: `BearerTokenStore` and `PairingService` are only instantiated in tests today — the engine's production composition wires the localhost-auto-accept `HandshakeHandler` ctor, never the LAN-mode pairing flow. Building active socket closure on revoke would mean wiring the full pairing infrastructure into `EngineComposition` (new IPC handler for revoke, per-connection bearer-hash tracking on `WebSocketTransport`, composition-root callback wiring) — that's not a closure-spec item, it's a separate "LAN pairing wireup" spec. Stays deferred pending that work.
