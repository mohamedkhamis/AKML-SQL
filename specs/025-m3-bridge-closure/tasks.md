...
- [X] T034 [US5] **Scaffolded 2026-05-28** (post-PR follow-on). `tests/AkmlSql.Web.E2E.Tests/UserStory2Tests.cs` written with 4 `[Fact(Skip=…)]` methods carrying full Playwright pseudocode for `LocalhostPair_FirstConnect_ReachesOpen`, `LocalhostPair_Reload_PreservesBearer`, `RevocationFails_RetryRespectsPinRequired`, `EngineKill_ReconnectRestoresLive`. Class methods all carry `[Trait("Category","BridgeE2E")]`. Skip lifts when an interactive session iterates selectors against the running app — the advisor flagged blind Playwright assertions as high-risk; the scaffold captures the shape (and the assertion text) without committing to selectors that might not match the actual DOM. The 4 tests build + skip cleanly in 26 ms. Wire-level coverage of the same scenarios already passes under T033's `BridgeHandshakeTests`.
