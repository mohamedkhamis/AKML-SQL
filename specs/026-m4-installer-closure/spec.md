...
- **FR-005**: The success page URL line MUST show `http://localhost:<IISPort>/` for localhost mode (with `:<IISPort>` omitted when the port is 80) and `https://<hostname>:<IISPort>/` for LAN mode (with `:<IISPort>` omitted when 80).
