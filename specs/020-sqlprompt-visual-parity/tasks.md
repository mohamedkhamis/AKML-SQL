...
- [X] T031 [US2] Extended `FormatRequestHandler` with `HandleProfileExportSqlPrompt` (paired with new IPC messages `ProfileExportSqlPrompt = 29` and `ProfileExportSqlPromptResult = 129`, request/response POCOs in `src/AkmlSql.Core/Ipc/Messages/`). Wrapper `ProfileExportSqlPromptHandler` in `Handlers/Formatting/FormattingHandlers.cs`, registered in `EngineHandlerRegistry.cs`. Validates the destination path with the same envelope `HandleBulkFormatAsync` uses (absolute + canonical, rejects traversal sequences). Loads the profile via `ProfileManager`, delegates to `SqlPromptExporter.ExportToFile` (atomic temp+rename + auto-creates directory). 6 new tests in `FormatRequestHandlerTests.cs` cover successful export, round-trip preservation, unknown profile, empty path, relative path, and path traversal. Full `AkmlSql.Engine.Tests` suite 1058/1058 pass.
...
- [X] T044 [US3] Audit complete — the file path in the original task (`src/AkmlSql.Shell.Shared/Options/OptionsDialog.xaml.cs`) doesn't exist; the real Options dialog is `src/AkmlSql.Shell.Shared/Dialogs/SettingsWindow.cs` (programmatic WPF, same pattern as spec-020's other reference impls). Compared its `_navTree` against SQL Prompt §1.2 — **no missing pages**. AKML's tree is a superset (adds an Editor ▸ Productivity/Navigation/Refactoring group and a Queries ▸ Execution leaf; renames "Prompt AI" → "AI Assistance" with the same scope). Inline gap-comment block written above `BuildSidebar`'s `AddTreeGroup` calls in `SettingsWindow.cs`, mapping each SQL Prompt page to its AKML page key and listing the AKML-only deviations.
- [X] T045 [P] [US3] Superseded — the pages this task names (Queries ▸ Execution Warnings, Queries ▸ Query Results, Miscellaneous ▸ Labs) already exist as `SafetyPage` / `GridPage` / `LabsPage` (instantiated in `SettingsWindow._pages` and wired into `_navTree` via the SQL Prompt-style tree). Authored in earlier specs (014 / Phase 2); T044 confirms they're real implementations rather than stubs. No new files needed.
- [X] T046 [US3] Visual chrome already matches the spec's intent in `SettingsWindow.cs`: window sized 880×620 with min 720×520 (lines ~229–232 — slight height/min ergonomic adjustments from the spec's 880×600/700×500, intentionally retained for long page labels like "Execution Warnings"), tree nav column width 240 DIU (line ~346 — comment explains the deviation from spec's 220), page-title accent and zebra row striping in place (page header at line ~1175 with "Restore Defaults" link on the right; striping helper referenced at line ~1218), button bar with Restore All Defaults / Import / Export present (lines 318–322). All chrome rendering goes through `ThemeManager` brushes (`_theme.FgPrimary`, `_theme.Sep`, etc.) per the spec-020 token rule. (Done in an earlier spec; the original task referenced the non-existent `OptionsDialog.xaml`.)
- [X] T047 [US3] Per-page Restore Defaults implemented in `SettingsWindow.cs`: the dispatch switch at lines 1559–1583 routes each page-key to a category-scoped reset against a fresh `AppSettings()`; the "Restore Defaults" link is rendered as part of the SQL Prompt-style page header (line ~1175). Lives as programmatic WPF rather than a separate `RestoreDefaultsLink.xaml` user control — matches the rest of the dialog's programmatic idiom.
- [X] T048 [US3] Restore All Defaults button + confirmation dialog present in `SettingsWindow.cs`: `MakeButton("Restore All Defaults", 140)` at line 319, click-wired to `OnResetAllClick` at line 1591 which calls `MessageBox.Show(..., MessageBoxButton.YesNo, MessageBoxImage.Warning)` and only proceeds on YES (lines 1595–1597), replacing `_settings` with a fresh `new AppSettings()` and reloading controls — atomic, all-or-nothing.
...
- [X] T059 [US3] **Implemented** per `specs/020-sqlprompt-visual-parity/T059-runbook.md`. Shared command id `CommandIds.CmdFormatStyles = 0x0916` in `PackageGuids.cs`; shared launcher `FormatStylesCommand` in `src/AkmlSql.Shell.Shared/Commands/` (registered in `AkmlSql.Shell.Shared.projitems`). Each host's VSCT (`AkmlSqlSsms20.vsct` ... `AkmlSqlVS2026.vsct`) gets a `<Button id="cmdFormatStyles" priority="0x0301">` under the existing `AkmlSqlMenuGroup` (top-level "AKML SQL" menu) plus the corresponding `<IDSymbol name="cmdFormatStyles" value="0x0916" />`. Each host's `AkmlSqlPackage.cs` invokes `FormatStylesCommand.Initialize(this, commandService)` immediately after `OptionsCommand.Initialize`. All 6 shell projects build clean via per-project MSBuild after a `obj/bin` reset (per the CLAUDE.md VSCT cross-contamination rule).
...
- [X] T065 [P] [US4] **Closed as already-served by `WildcardExpansionPopup`** — re-confirmed in the 2026-05-23 evening session. The SQL Prompt "column picker modal" UX (checkbox list of columns grouped by table, double-click commit, Tab/Enter shortcuts) is implemented in `src/AkmlSql.Shell.Shared/Editor/Completion/WildcardExpansionPopup.cs` and chrome flows through `ThemeRegistry` (`EditorPopupBackground` / `EditorPopupBorder` / `TextSecondary` / `SurfaceCanvas`). It's invoked from `CompletionController.cs` when the user presses Tab on a `*` wildcard. The `CmdColumnPickerOpen = 0x0900` reservation in `PackageGuids.cs` is from spec-019's catalog and remains free; wiring it to an alternate keyboard-shortcut entry point into the same popup is a small follow-up if needed, but not required for spec-020's parity goal. A separate `ColumnPickerWindow` *modal* (separate window, blocks the editor) was the spec author's mistaken assumption — SQL Prompt's column picker is a non-modal in-editor popup, which AKML already matches.
...
- [X] T070 [US5] **Implemented**: extended `FormatPreviewResponse` with a new `ValidationError` field ([src/AkmlSql.Core/Ipc/Messages/FormatPreviewResponse.cs:13](src/AkmlSql.Core/Ipc/Messages/FormatPreviewResponse.cs#L13), `Key(2)`). Populated by `FormatRequestHandler.HandleFormatPreview` whenever `FormatResult.ValidationPassed == false` (the engine returns the original SQL unchanged on stage-6 failure, so `ValidationError` is the only signal the editor has). Editor view model exposes a notifying `PreviewValidationError` property; `FormatStylesEditorWindow` renders an amber warning bar above the preview pane that toggles visibility on the property change. Wire contract documented in `specs/020-sqlprompt-visual-parity/contracts/ipc-format-preview-debounce.md` (unchanged — the field name matches the contract).

### Parity corpus + driver

- [X] T071 [US5] **Implemented as the swap-in document** at [tests/format-parity/README.md](tests/format-parity/README.md). Documents the Redgate-CLI generation path (`SqlPrompt.Format.CommandLine.exe`), the `.json` vs `.sqlpromptstylev2` wire-format gotcha (CLI uses `.json`, editor uses XML), the `--create-backups` / in-place-format flow, and the directory layout the driver expects. A future trial-run session can follow the document step-by-step. No PowerShell `assemble-corpus.ps1` shipped — the Redgate CLI is invoked directly per the README; a separate orchestration script would just be ceremony around the four shell commands documented inline.
- [X] T072 [US5] **Populated with AKML self-goldens** (13 hand-crafted SQL inputs × 6 built-in styles = 78 (input, style) pairs in `tests/format-parity/{corpus,golden}/`). Inputs cover the formatter's interesting paths: simple SELECT, multi-join, CTE with column list, multiple CTEs, searched CASE, simple CASE, short / long IN-list, BETWEEN + operators, DDL with constraints, stored procedure, MERGE, correlated subqueries. The drift-guard form ships now; the Redgate-formatted form lands once someone runs the swap-in path documented in T071.
- [X] T073 [US5] **Implemented** at [tests/AkmlSql.Formatting.Tests/Parity/FormatParityTests.cs](tests/AkmlSql.Formatting.Tests/Parity/FormatParityTests.cs). `[Theory]` driver enumerates `corpus/*.sql × built-in styles` pairs; capture-vs-compare mirrors the `PerformanceBaselineTests.Capture_or_compare_M0_baseline` pattern (writes golden on miss / `AKML_UPDATE_PARITY_GOLDEN=1`, asserts byte-exact equality otherwise). `Normalise` helper applies the SC-007 / Q1 rules (strip trailing whitespace per line, normalise `\r\n` / `\r` → `\n`, drop UTF-8 BOM) and is idempotent. 78 / 78 pairs pass in compare mode. Same driver will become the SC-007 parity measurement when Redgate goldens replace the AKML self-goldens — no driver change required.
...
- [X] T080 [P] [US5] **Implemented end-to-end**. Round-trip: `Cte.PlaceColumnsOnNewLine` string-enum property in `CteOptions` ("ifLongerThanWrap" default / "always" / "never"); importer normalises `PlaceCteColumnsOnNewLine` values; exporter emits the SQL Prompt PascalCase token; `FormatSettingSchema.ExplicitKeyMap` flips the setting's Status to `Implemented`. Layout: `ControlFlowRules.ApplyCteColumnListPlacement` detects the optional column-list parens between `<CteName>` and `AS` (handles both `WITH name (col1, ...) AS (...)` and `WITH name AS (...)` shapes), then applies placement: `always` → forces opening paren onto a new line at `withIndent+1`; `never` → forces inline; `ifLongerThanWrap` → wraps only when measured length + ~20 char margin exceeds `Whitespace.MaxLineWidth`.
...
- [X] T082 [P] [US5] **Implemented end-to-end**. Round-trip: 3 properties on `CaseOptions` — `FirstWhenOnNewLine` (string enum: "auto" / "always" / "never"), `WhenAlignment` (string enum: "toCase" / "toFirstItem" / "indentedFromCase"), `ExpressionOnNewLine` (bool); importer / exporter handle `PlaceFirstWhenOnNewLine` / `WhenAlignment` / `PlaceCaseExpressionOnNewLine`. Layout: `ControlFlowRules.ApplyCaseRules` now (a) tracks the first WHEN separately and applies `FirstWhenOnNewLine` override on top of the existing `WhenOnNewLine`; (b) resolves the indent level via `ResolveWhenIndent(caseOpts, caseIndent)` so `WhenAlignment` chooses between toCase/indentedFromCase ("toFirstItem" is approximated as "toCase" — true column alignment would require post-emission column measurement, documented in the helper's `<remarks>`); (c) when `ExpressionOnNewLine = true`, places the simple-CASE expression on a new line below CASE. 5 new unit tests cover the new behaviour.
- [X] T083 [P] [US5] **Implemented end-to-end**. Round-trip: new `OperatorsOptions` class with `Alignment` (string enum: "inlineWithStatement" / "indentedFromStatement" / "rightAligned") and `BetweenOnNewLine` (bool); importer / exporter handle `OperatorsAlignment` and `PlaceBetweenKeywordOnNewLine`. Layout: new `ControlFlowRules.ApplyOperatorRules` — when `Alignment != "inlineWithStatement"`, bumps the `IndentLevel` of every AND/OR token already on its own line by +1 (true right-aligned column alignment falls back to indentedFromStatement at the layout layer, per the same documented limitation as T082's "toFirstItem"); when `BetweenOnNewLine = true`, forces a line break before `BETWEEN`. 4 new unit tests cover the new behaviour.
- [X] T084 [P] [US5] **Implemented end-to-end**. Round-trip: new `InStatementsOptions` class with single `Alignment` property (string enum: "stacked" / "wrapped" / "rightAligned"); importer / exporter handle `InStatementsAlignment`. Layout: new `ControlFlowRules.ApplyInStatementsAlignment` — when `Alignment = "wrapped"` and the IN list is already multi-line, re-flows items as a width-bounded paragraph (packs multiple items per line up to ~80 chars; each comma stays inline followed by a single space unless the next item would push past the budget, in which case it wraps to a new line indented one level from the opening paren). `"stacked"` keeps the existing one-item-per-line layout from `ExpandToMultiLine`; `"rightAligned"` falls back to `"stacked"` at the layout layer.
- [X] T085 [US5] **Done by T080 + T082 + T083 + T084** — every newly-added SQL Prompt key has matching importer / exporter entries plus an `ExplicitKeyMap` entry in `FormatSettingSchema`, so the corresponding settings now report `Status = Implemented` in `RequestStyleEditorSchema` (msg 28) responses. Verified by `FormatSettingSchema.Default.Settings.Where(s => s.Status == "Implemented")` covering the new fields after schema rebuild.
