...
- [ ] T056 [P] [US3] Right-top — **placeholder only**. Shows the selected setting's metadata (`Id`, `Type`, `Status`, `Default`, `SqlPromptKey`) but no editable controls yet. Type-driven controls (CheckBox / ComboBox / IntegerSpinner) deferred to **Tier 2b**.
- [ ] T057 [P] [US3] Right-bottom — **placeholder only**. The bordered region exists and is theme-aware; the actual live `FormatPreview` IPC consumer + 100 ms debounce + supersession + 200-line sample SQL all deferred to **Tier 2b**.
