...
- [ ] T023 [US2] Add 12 new sub-settings POCOs to `src/AkmlSql.Engine/Formatter/Profiles/FormatProfile.cs` (`WhitespaceSettings`, `ListSettings`, `ParensSettings`, `CasingSettings`, `DmlSettings`, `DdlSettings`, `ControlFlowSettings`, `CteSettings`, `JoinSettings`, `CaseSettings`, `OperatorSettings`, `InStatementSettings`) — each with `[JsonPropertyName]` matching the `.sqlpromptstyle` JSON shape
- [ ] T024 [US2] Add `[JsonExtensionData] public Dictionary<string, JsonElement> PassthroughUnknownKeys { get; set; }` to `FormatProfile` AND to each sub-settings POCO so unknown nested keys round-trip at their original JSON paths
- [ ] T025 [P] [US2] Implement `SqlPromptKeyMap` in `src/AkmlSql.Engine/Formatter/Profiles/SqlPromptKeyMap.cs` — a static `IReadOnlyList<KeyMapEntry>` with the 33+1 entries from `data-model.md` (each entry = SqlPromptKey, AkmlField, Type, Transform, Status)
- [ ] T026 [P] [US2] Implement `FormatSettingSchema` in `src/AkmlSql.Engine/Formatter/Profiles/FormatSettingSchema.cs` — built from `SqlPromptKeyMap` + AKML-native-only settings, exposes ordered `Groups[]` and flat `Settings[]`

### Engine — Importer / Exporter

- [ ] T027 [US2] Implement `SqlPromptStyleImporter` in `src/AkmlSql.Engine/Formatter/Profiles/SqlPromptStyleImporter.cs` — `Path.GetFullPath` canonicalisation, 1 MB file-size cap, JSON parse via `System.Text.Json`, populate `PassthroughUnknownKeys` for keys not in `SqlPromptKeyMap`, return `ImportResult { Profile, UnsupportedSettings[], PassthroughKeys[] }`. No partial state on failure.
- [ ] T028 [US2] Implement `SqlPromptStyleExporter` in `src/AkmlSql.Engine/Formatter/Profiles/SqlPromptStyleExporter.cs` — single `JsonSerializer.Serialize` pass; `JsonExtensionData` flushes pass-through keys at their original section paths automatically
- [ ] T029 [US2] Extend `FormatRequestHandler.HandleProfileImport` in `src/AkmlSql.Engine/Server/FormatRequestHandler.cs` — branch on `Path.GetExtension(canonicalPath)`: `.akmlstyle` → existing importer; `.sqlpromptstyle` → `SqlPromptStyleImporter`; anything else → `Error` with code `UNSUPPORTED_EXTENSION`
- [ ] T030 [US2] Extend `ProfileImportResponse` POCO with new optional fields `UnsupportedSettings : string[]?`, `PassthroughKeys : string[]?`, `Kind : string?` per `contracts/ipc-profile-import-sqlprompt.md`; MessagePack backward-compatible (additional `[Key(N)]` slots; existing clients ignore missing values)
- [ ] T031 [US2] Extend `FormatRequestHandler.HandleProfileSave` to detect `.sqlpromptstyle` target extension and route through `SqlPromptStyleExporter` instead of the native exporter

### Engine — Built-in styles & active-style scope

- [ ] T032 [P] [US2] Transcribe Redgate "Compact" style into `src/AkmlSql.Engine/Formatter/Profiles/BuiltIns/CompactStyle.cs` (static factory returning a `FormatProfile` with `IsReadOnly = true`, name `"Compact (Redgate)"`)
- [ ] T033 [P] [US2] Transcribe Redgate "Indented" style into `src/AkmlSql.Engine/Formatter/Profiles/BuiltIns/IndentedStyle.cs`
- [ ] T034 [P] [US2] Transcribe Redgate "AlignedLeftBracket" style into `src/AkmlSql.Engine/Formatter/Profiles/BuiltIns/AlignedLeftBracketStyle.cs`
- [ ] T035 [US2] Implement `BuiltInStyleSeeder` in `src/AkmlSql.Engine/Formatter/Profiles/BuiltInStyleSeeder.cs` — on first launch, writes each built-in into `%AppData%/AKML SQL/styles/`-equivalent with `IsReadOnly=true`; idempotent (never overwrites a user-modified read-only style — if the file exists, skip); writes a `builtIns.v1.seeded` marker
- [ ] T036 [US2] Wire `BuiltInStyleSeeder` into engine startup (after `ConfigManager.Load`, before `PipeRpcServer` starts accepting clients)
- [ ] T037 [US2] Confirm `FormatterSettings.ActiveProfile` in `src/AkmlSql.Core/Config/AppSettings.cs` remains a single global string (it already is) and add an XML-doc comment citing FR-027b: "Global per user across all hosts — never split per-host"
- [ ] T038 [US2] Add a "FormatStyle fork-to-Native" helper in `src/AkmlSql.Engine/Formatter/Profiles/FormatStyleForker.cs` that takes a read-only style and returns a new writable `FormatProfile` with a fresh `Id`, name suffix `" (copy)"`, and `Kind = Native`

### Tests

- [ ] T039 [P] [US2] Implement `SqlPromptStyleImporterTests` in `tests/AkmlSql.Core.Tests/Format/SqlPromptStyleImporterTests.cs` covering: real-world style file succeeds; unknown key preserved in passthrough; unsupported key (`casing.useObjectDefinitionCase`) surfaces in response; malformed JSON returns error with no partial state; oversize (> 1 MB) rejected; path-traversal payload rejected
- [ ] T040 [P] [US2] Implement `SqlPromptStyleExporterTests` in `tests/AkmlSql.Core.Tests/Format/SqlPromptStyleExporterTests.cs` covering: round-trip preserves unknown keys at original JSON paths; export of a built-in produces JSON that matches the `.sqlpromptstyle` schema; edits to known fields survive round-trip
- [ ] T041 [P] [US2] Implement `SqlPromptKeyMapTests` in `tests/AkmlSql.Core.Tests/Format/SqlPromptKeyMapTests.cs` — every entry has a defined default; no duplicate `SqlPromptKey`; every `Status=Implemented` entry maps to an existing `FormatProfile` field reachable via reflection
- [ ] T042 [US2] Add `BuiltInStyleSeederTests` to the same folder — idempotent re-seed; user-modified read-only file is not overwritten; markers respected
- [ ] T043 [US2] Add a global-active-style test (`ActiveProfileScopeTests`) verifying `ConfigManager.Load` reads a single `ActiveProfile` and the value is unaffected by host identity
...
- [ ] T052 [US3] Create `FormatStylesEditorWindow.xaml` and `.xaml.cs` in `src/AkmlSql.Shell.Shared/Formatting/` — `Window` subclass, `WindowStartupLocation = CenterOwner`, owner set via DTE HWND in `Loaded` (follows `HistoryDiffWindow` reference)
- [ ] T053 [US3] Implement three-column `Grid` with `GridSplitter` between columns; widths: 240 / * / 360 DIU default; right column itself splits vertically 60 / 40 for controls / preview
- [ ] T054 [P] [US3] Implement `StyleListPanel.xaml` (left) in `src/AkmlSql.Shell.Shared/Formatting/` — virtualised list of styles, badge for `Kind`, lock icon for `IsReadOnly`, radio for `IsActive`, Import / Export / Create / Copy / Delete / Fork-to-Native buttons
- [ ] T055 [P] [US3] Implement `SettingTreePanel.xaml` (middle) — `TreeView` built from `FormatSettingSchema.Groups`; each leaf is a setting; unsupported settings render with `IsEnabled=false` and a "not yet supported" `Badge` user control adjacent (FR-023)
- [ ] T056 [P] [US3] Implement `SettingControlsPanel.xaml` (right-top) — type-driven control selection (`CheckBox` for Bool, `ComboBox` for Enum, `IntegerSpinner` for Int/IntRange); each control is `IsEnabled = setting.Status != Unsupported`
- [ ] T057 [P] [US3] Implement `LivePreviewPanel.xaml` (right-bottom) — read-only Avalon-style code preview; consumes `FormatPreviewResult`; shows inline warning bar on `ValidationError`
- [ ] T058 [US3] Implement `FormatStylesEditorViewModel.cs` — orchestrates import / export / activate / fork / preview; talks to engine via existing `IPipeClient`; depends on T054–T057
- [ ] T059 [US3] Wire Options → Format → Styles → "Edit Formatting Styles…" button to launch `FormatStylesEditorWindow.ShowDialog`
- [ ] T060 [US3] Implement the "Settings not yet supported" badge user control in `src/AkmlSql.Shell.Shared/Formatting/UnsupportedBadge.xaml` — small pill, tooltip "This setting is in your imported file but AKML-SQL doesn't honour it yet. It will be preserved on export."
...
- [ ] T061 [P] [US4] Re-skin suggestion popup container in `src/AkmlSql.Shell.Shared/Completion/CompletionPopup.xaml` to use `Editor.PopupBackground`, `Editor.PopupBorder`, `Editor.PopupShadow` tokens (drop any remaining hardcoded `#252836`)
- [ ] T062 [US4] Apply `IconBadge.*` tokens to suggestion items in `src/AkmlSql.Shell.Shared/Completion/SuggestionItemRenderer.cs` — switch on object type, resolve background = 20%-alpha brush, foreground = solid brush
- [ ] T063 [US4] Implement Ctrl-held semi-transparency on suggestion popup (`KeyDown`/`KeyUp` hook on the editor; sets `Opacity = 0.6` while Ctrl is down; never below 0.4)
- [ ] T064 [P] [US4] Re-skin object-definition side panel in `src/AkmlSql.Shell.Shared/Completion/ObjectDefinitionPanel.xaml` — Summary/Script tabs, hoisted `Consolas` font for Script tab, all chrome via tokens
- [ ] T065 [P] [US4] Re-skin column picker modal in `src/AkmlSql.Shell.Shared/Completion/ColumnPickerWindow.xaml` — checkbox rows, key-icon badges (PK/FK/UQ), sort toggle, Select All, keyboard handlers (Space toggle / Enter accept / Esc cancel)
- [ ] T066 [P] [US4] Re-skin `SnippetManagerDialog` (already migrated off legacy `ThemeManager` by T021) to match `08_column_picker_snippets.svg` layout — list left, preview right, metadata-row bottom
