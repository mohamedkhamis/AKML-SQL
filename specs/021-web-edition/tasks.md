...
- [ ] T132 [US5] Add `src/AkmlSql.Web/Shared/AiChatPanel.razor`: free-form conversational panel with message history (in-memory only — no persistence) *(deferred -- the four action buttons in AiPanel cover the bulk of US5 acceptance; the free-form chat is a v2 affordance. The AiClientFactory.SendAsync surface already supports multi-turn (it takes a system + user prompt; extending to a message array is ~10 LOC), the UI surface lands when an interactive session can verify the conversational UX.)*
