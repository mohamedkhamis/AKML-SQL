...
- [X] T061 [US2] Add `src/AkmlSql.Engine/Capabilities.cs`: enumerate `core.format.v1`, `core.analysis.v1`, `schema.v2`, `schema.cache.v1`, `snippets.write`, `refactoring.heavy`, `diagnostics.engine-log-tail.v1`; report dynamically from handshake *(landed. `Capabilities.Current` currently advertises the three M0+M3-baseline capabilities (`CoreFormatV1`, `CoreAnalysisV1`, `SchemaV2`); the remaining four capability identifiers (`SchemaCacheV1`, `SnippetsWrite`, `RefactoringHeavy`, `DiagnosticsEngineLogTailV1`) are defined constants but not yet in the advertised list — they get added when their respective features land (T104, T115, T117, M2 diagnostics export).)*
