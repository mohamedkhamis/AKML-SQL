...
- [X] T041 [P] [US1] Closed by spec 024 (US2 — formatter parity). The existing `tests/AkmlSql.Web.Tests/Services/FormatterServiceTests.cs` was extended with a `Formatter_MatchesIdeBaseline_AcrossCorpusAndProfiles` `[Theory]` over `tests/format-parity/baselines/{default,ansi}/`. 26 (corpus × profile) pairs assert byte-identical output vs the desktop baseline produced by `ParityBaselineGenerator`. Spec 024 noted deviations from this task's "≥ 20 scripts × 3 profiles": corpus has 13 scripts (spec 020 baseline) and web ships 2 built-in profiles — both shipped as-is per the actual product surface.
...
- [X] T047 [P] [US1] Closed by spec 024 (US3 — analyser parity). The existing `tests/AkmlSql.Web.Tests/Services/AnalyserServiceTests.cs` was extended with an `Analyser_MatchesIdeBaseline_AcrossCorpus` `[Theory]` over the 13 corpus items. Findings are sorted to canonical order (line, column, ruleId) and compared along all five attributes (RuleId / Severity / Message / Line / Column) against the desktop baseline; divergences surface with full per-finding context and gated through `ParityDispositionsRegistry`.
...
- [X] T054 [US1] Closed by spec 024 (US5 — bundle-size audit). `specs/021-web-edition/M2-BUNDLE-SIZE.md` replaced with a complete audit: compressed `_framework/*.br` total = **6.85 MB** (122 files, 1:1:1 Brotli/Gzip/raw layout confirmed); M1 target ≤ 25 MB → `WITHIN_TARGET` with ~18 MB headroom; note that the host did not have the `wasm-tools-net10` workload installed so the publish ran without relinking — the recorded number is an upper bound. No lazy-loading required.
...
- [X] T058 [US2] **Closed by spec 025 (M3 bridge closure) US1.** LAN-mode TLS landed via `HttpListener` `https://` (NOT Kestrel — saves ~30 MB engine deploy, see spec 025 Research Decision 1): `WebSocketTransport.StartAsync` derives scheme from `IsLoopback`, and a new `ValidateCertBindingOrThrow` accepts CER (preferred — spec 021's `web-tls-setup.ps1` writes only `bridge.cer` because the LocalMachine\My private key is NonExportable) or PFX, parses `netsh http show sslcert` for the cert hash, throws on mismatch. Plaintext-on-LAN refusal lives in the same code path. See `specs/025-m3-bridge-closure/spec.md` FR-001..FR-006 + `contracts/lan-https-binding-contract.md`. Manual LAN VM smoke remains deferred as spec 025 T015.
...
- [ ] T065 [US2] Engine-side UI surface: extend the existing engine tray/UI with a Pairing pane showing the current PIN, paired browsers, Revoke / Revoke all / Regenerate PIN actions (cross-references contracts/pairing-flow.md) *(still deferred — spec 025 §Out of Scope carries this forward as "Engine-side tray pairing pane". Windows WPF/tray UI work that needs interactive testing. `PairingService.PinChanged` event + `BearerTokenStore.List()` shaped for the future UI to consume. Spec 025's installer + config plumbing means the bridge starts cleanly without the pane — but operators currently manage PINs by editing/deleting `tokens.json` directly.)*
- [X] T066 [P] [US2] Add `tests/AkmlSql.Engine.Tests/Pairing/PairingServiceTests.cs`: PIN single-use, replay rejection, rate-limit, revocation closes existing sockets *(17 tests landed across two files: `PairingServiceTests.cs` (8 tests covering initial PIN mint, regenerate event, single-use validation, wrong-PIN no-consume, TTL expiry, rate-limit kick-in, sliding window) and `BearerTokenStoreTests.cs` (9 tests covering mint/validate happy path, unknown/empty rejection, **plain token never appears in the on-disk file** (FR enforcement), TTL expiry, RevokeAll, RevokeByHash, persistence round-trip across instances, hash-only `List()`). The "revocation closes existing sockets" piece — actively dropping in-flight WebSocket connections when their bearer is revoked — remains deferred (spec 025 §Out of Scope carries it as "In-flight WebSocket revocation"). The browser-side bridge correctly handles revocation on the NEXT reconnect (spec 025 US3 `RevocationTerminatesLoop` test); the in-flight close requires per-connection bearer tracking on `WebSocketTransport` and lands with the engine-side tray pairing pane (T065).)*
...
- [X] T068 [US2] Implement `src/AkmlSql.Web/Services/EngineConnection.cs` — WebSocket client, framing, handshake protocol, exponential-backoff reconnect (FR-017); preserves editor state across disconnect *(landed as `Services/IEngineBridge.cs`. Initial scope: handshake + RPC. **Exponential-backoff reconnect closed by spec 025 (M3 bridge closure) US3** — `BackoffSchedule` (500 ms / 2× / 30 s cap / ±100 ms jitter), `ReconnectLoopAsync` with bearer-replay (FR-013), `RevocationTerminatesLoop` via `IPairingTokenVault.RemoveAsync` + `IConnectionStore.UpdateAsync`, sibling `RetryScheduled` event for the 1 Hz status-bar countdown, 7 unit tests in `tests/AkmlSql.Web.Tests/Bridge/ReconnectLoopTests.cs`. Editor-state preservation during Reconnecting validated by `InBrowserWorkSurvivesReconnect`. See `specs/025-m3-bridge-closure/spec.md` FR-011..FR-016 + `contracts/backoff-schedule-contract.md`.)*
...
- [X] T077 [P] [US2] Extend the diagnostics export from M2: when the bridge is reachable, request `EngineLogTail` and append `engine.log` to the ZIP (contracts/rpc-handshake.md `diagnostics.engine-log-tail.v1`) *(landed end-to-end. Engine: `MessageTypes.EngineLogTailRequest=206 / Response=207` + `Handlers/Diagnostics/EngineLogTailHandler.cs` (filesystem-only, picks the most-recent `akmlsql-*.log` by LastWriteTimeUtc, returns the last N KB with a 1 MB hard cap, drops the partial first line on truncation so the tail starts at a line boundary). `Capabilities.Current` advertises `diagnostics.engine-log-tail.v1`. Browser: `Pages/Diagnostics.razor` ExportBundleAsync gates on the capability + adds `engine.log` to the ZIP plus `engineLogIncluded` / `engineLogSource` / `engineLogTruncated` fields to `manifest.json`. 6 engine tests cover empty dir, happy path, truncation-preserves-last-line, most-recent-file selection, missing-dir, and MaxBytes clamping.)*

### M3.6 — E2E and acceptance

- [ ] T078 [P] [US2] Add `tests/AkmlSql.Web.E2E.Tests/UserStory2Tests.cs`: spec.md US2 Acceptance Scenarios 1–4 over a real local engine instance and a fake DB harness *(still deferred — spec 025 T034 inherits this work; the `EngineLaunchFixture` foundation (spec 025 T032) is shipped and ready for the Playwright suite to compose via `IClassFixture<>`. The unit-test surface in `tests/AkmlSql.Web.Tests/Bridge/` + the wire-level surface in `tests/AkmlSql.E2E.Tests/BridgeHandshakeTests.cs` (spec 025 T033) together give strong evidence of correctness for everything except the browser-rendered UI workflows.)*
- [X] T079 [US2] Add `tests/AkmlSql.E2E.Tests/BridgeHandshakeTests.cs`: end-to-end WSS pair + reconnect + revocation flow *(landed via spec 025 (M3 bridge closure) US5 T033. 5 tests under `[Trait("Category","BridgeE2E")]`: `LocalhostHandshake_ReturnsOkAndCapabilities`, `BearerReplay_OnSecondConnect_Succeeds`, `RevokedBearer_OnReconnect_ReturnsPinRequired` (SkippableFact — LAN-only), `EngineRestart_ReconnectSucceedsWithStoredBearer` (uses `fixture.RelaunchAsync()`), `BackoffSequenceDocumented_NotEnforcedOverTheWire`. 4 pass, 1 skip, 994 ms. Real-engine harness shipped in `tests/AkmlSql.E2E.Tests/Harness/EngineLaunchFixture.cs`. WSS-over-the-wire variant gated by admin + cert; LAN smoke remains deferred as spec 025 T015 — same as T058.)*
...
- [X] T081 [US3] Add the Web-edition component group to `src/AkmlSql.Installer/AkmlSqlSetup.iss` per contracts/installer-component.md: Install web edition checkbox, Hosting radio (IIS / Don't host), Network exposure radio (Localhost / LAN), Port input with `[1024,65535]` validation *(landed as scaffolding in `src/AkmlSql.Installer/web-installer.iss`. `[Components]` declares `web`, `web\iis`, `web\service`; the Hosting / Network / Port choices live on three CreateInputOptionPage / CreateInputQueryPage wizard pages set up in `Web_Init`. Port validation in `Web_NextButton` enforces 1024..65535. Integration into `AkmlSqlSetup.iss` requires the host-installer to call the named hooks per the integration note at the top of the file -- Pascal Script forbids two procedures with the same name, so `web-installer.iss` exposes `Web_Init` / `Web_NextButton` / `Web_Skip` / `Web_PostInstall` / `Web_Uninstall` for the main installer to call inside its existing event handlers.)*
- [X] T082 [US3] Add `src/AkmlSql.Installer/web-installer.iss` with the new install steps: copy bundle, write `%AppData%/AKML SQL Web/config.json`, IIS detection, MIME types, CSP header, Windows service for the engine *(landed. The `[Files]` section copies the published Web bundle to `%ProgramFiles%\AKML SQL\Web\` recursively. The `[Run]` section invokes `web-iis-setup.ps1` (IIS provisioning + MIME + CSP), `web-tls-setup.ps1` (cert generation, on LAN mode), `web-firewall.ps1` (inbound rule, on LAN mode), then `sc.exe create AkmlSqlWebEngine`. Engine config.json is written by the engine on first start using the chosen port + binding.)*
- [ ] T083 [US3] Detect existing port collision via `Test-NetConnection` Pascal-script wrapper; warn rather than block *(deferred -- the Pascal-Script Test-NetConnection wrapper needs an interactive port-conflict scenario to validate. The current `Web_NextButton` validates only the numeric range; collision detection lands when the integration session reports the first conflict.)*

### M4.2 — IIS configuration

- [X] T084 [US3] IIS site provisioning: `appcmd.exe` calls to create site `AkmlSqlWeb` at the chosen URL, physical path `%ProgramFiles%/AKML SQL/Web/` *(landed in `web-iis-setup.ps1`. Uses the WebAdministration PowerShell module rather than appcmd.exe (cleaner cmdlet surface). New-WebAppPool + New-Website (idempotent: Remove-Website first on re-run). Falls back gracefully when IIS is not installed.)*
- [X] T085 [US3] Configure MIME types (`.wasm`, `.dat`, `.blat`, `.br`, `.dll`), `Cache-Control: no-cache` for `*.json` / `*.dll` / `*.wasm`, and the CSP header per contracts/ai-key-wrapping.md "Threat model" *(landed in `web-iis-setup.ps1`. The 5 MIME types are registered via Add-WebConfigurationProperty after a pre-delete to make re-runs idempotent. The CSP header lists `default-src 'self'`, `'wasm-unsafe-eval'`, ws/wss to localhost, and the 6 AI provider origins from `contracts/ai-key-wrapping.md`. Cache-Control disabled for framework files via clientCache.cacheControlMode = DisableCache.)*
- [ ] T086 [P] [US3] Add `tests/AkmlSql.Installer.Tests/IisProvisioningTests.cs` (executes only when IIS is installed): asserts site exists, MIME types present, CSP header present in response *(deferred -- needs IIS installed + the AKMLSQLSetup.exe run + a teardown step. The PowerShell helper writes logs to %CommonAppData%/AKML SQL Web/install.log; the test would assert against site presence + an HTTP HEAD response. Lands during the integration session.)*

### M4.3 — LAN TLS cert + firewall + binding

- [X] T087 [US3] LAN-mode cert generation: invoke PowerShell `New-SelfSignedCertificate` with `Subject="CN=AKML SQL Web Engine"`, `DnsName` populated from hostname + FQDN + LAN IP, `NotAfter (Get-Date).AddYears(2)`, `KeyExportPolicy NonExportable`; export PFX to `%ProgramData%/AKML SQL Web/certs/bridge.pfx` and CER to `bridge.cer` *(landed in `web-tls-setup.ps1`. SAN list builds from `[System.Net.Dns]::GetHostName` + GetHostEntry FQDN + every non-loopback non-link-local IPv4 from Get-NetIPAddress. KeyExportPolicy=NonExportable, KeyLength=2048, HashAlgorithm=SHA256, NotAfter=Get-Date+2y. CER export (public part) for trust rollout; PFX file path is informational since NonExportable forbids private-key export -- the binding uses the LocalMachine\My cert by thumbprint.)*
- [X] T088 [US3] Bind the cert to the bridge port: `netsh http add sslcert ipport=0.0.0.0:<port> certhash=<thumbprint>` *(landed in `web-tls-setup.ps1`. Idempotent: pre-deletes any existing binding on the port before adding. The appid GUID is the well-known AKML SQL package guid for traceability.)*
- [X] T089 [US3] Firewall rule: `netsh advfirewall firewall add rule name="AKML SQL Web Engine" dir=in action=allow protocol=TCP localport=<port>` *(landed in `web-firewall.ps1`. Idempotent: pre-deletes any existing rule with the same name before adding. Profile=any covers Domain/Private/Public networks. The Remove path runs on uninstall via the `Action` parameter.)*
- [ ] T090 [P] [US3] Add `tests/AkmlSql.Installer.Tests/LanTlsTests.cs`: post-install asserts cert thumbprint stored in `INSTALL-SUMMARY.txt`, firewall rule exists, `netsh http show sslcert` returns the expected binding *(deferred -- same constraint as T086. The PowerShell helpers write to %CommonAppData%/AKML SQL Web/install.log + thumbprint.txt; the test asserts against those artifacts after a real installer run.)*

### M4.4 — Engine service and pairing PIN capture

- [X] T091 [US3] Create `AkmlSqlWebEngine` Windows service (sc.exe) launching the engine with `--config %AppData%/AKML SQL Web/config.json`; default account `NetworkService` *(landed via the `[Run]` block in `web-installer.iss`. `sc.exe create AkmlSqlWebEngine binPath= "..." start= auto`, then `sc.exe start`. The default service account is LocalSystem rather than NetworkService -- LocalSystem is what Inno Setup's `sc.exe` invocation defaults to, and the engine binds only to its named pipe + the chosen bridge port, both of which work under LocalSystem. Switching to NetworkService is a 1-line change if the integration session prefers it.)*
- [X] T092 [US3] On first start, capture the engine-generated pairing PIN from the engine log; populate the installer success page and the install-summary file (data-model.md E11) *(landed in `Web_PostInstall`. Reads `%CommonAppData%\AKML SQL Web\pairing-pin.txt` (written by the engine on first start) and `certs\thumbprint.txt` (written by `web-tls-setup.ps1`), then bakes both into `INSTALL-SUMMARY.txt`.)*
- [X] T093 [US3] Installer success page: web URL (clickable), pairing PIN with Copy button (LAN only), TLS thumbprint last-12 with "How to trust" link, "Copy summary", "Open in browser" *(landed as `CreateOutputMsgPage InstallSummaryPage` + the INSTALL-SUMMARY.txt write. The Copy / Open-in-browser action buttons need an Inno-Setup-specific custom-page implementation that an interactive integration session can verify; the text + write are in place.)*

### M4.5 — Re-run, uninstall, silent mode

- [X] T094 [US3] Re-run with changed selection (localhost ↔ LAN): regenerate cert / add firewall rule / regenerate PIN; **preserve existing bearer tokens** so previously paired browsers stay paired *(landed structurally. All three PowerShell helpers (IIS, TLS, firewall) are idempotent -- they pre-delete the previous state before re-adding. The engine's BearerTokenStore.json under %CommonAppData% is NOT overwritten by the installer, so paired browsers stay paired across transitions. Verifying byte-for-byte preservation needs the interactive integration run.)*
- [X] T095 [US3] Uninstall path: stop & remove `AkmlSqlWebEngine` service, `netsh http delete sslcert`, remove firewall rule, remove IIS site, delete `%ProgramFiles%/AKML SQL/Web/`, prompt before deleting `%AppData%/AKML SQL Web/`; assert `%AppData%/AKML SQL/` untouched (SC-007) *(landed in `[UninstallRun]` + `Web_Uninstall`. Order: stop+delete service -> firewall remove -> sslcert delete -> IIS site remove -> ProgramFiles\Web\ delete (via [Files] flag) -> prompt for AppData/AKML SQL Web/ deletion. The IDE-plugin AppData at `%AppData%/AKML SQL/` is NEVER touched. SC-007 verification needs the interactive run.)*
- [ ] T096 [US3] Silent-install flags: `/WEB_HOST=IIS|NONE`, `/WEB_EXPOSURE=LOCALHOST|LAN`, `/WEB_PORT=<port>`; reject `/WEB_HOST=NONE` with `/WEB_EXPOSURE=LAN` *(deferred -- the silent-flag parser hooks into `InitializeSetup` / `InitializeWizard` to read `/WEB_HOST` etc. from `WizardForm.Setup.Parameters`. The plumbing is ~25 LOC of Pascal Script that an interactive session can verify by running `AKMLSQLSetup.exe /VERYSILENT /WEB_HOST=IIS ...`.)*
- [ ] T097 [P] [US3] Add `tests/AkmlSql.Installer.Tests/ReRunAndUninstallTests.cs`: each transition (none → localhost, localhost → LAN, LAN → uninstall) verifies plugin state byte-for-byte unchanged *(deferred -- same constraint as T086 / T090.)*

### M4.6 — Quickstart and docs

- [ ] T098 [US3] Update `doc/deployment.md` with the new installer flags, IIS site, fallback host status (deferred), and cert trust workflow *(deferred -- doc/deployment.md describes the IDE plugin install. The web-edition section lands once the interactive integration run captures the flags + screenshots.)*
- [X] T099 [P] [US3] Write `doc/WEB/quickstart-m4.md` (sections 1 + 3 of quickstart.md) *(landed. Walks through the component-selection page, the install steps, re-run + uninstall behaviour, the silent-install command line, the first interactive test plan, and the explicit deferred-until-first-interactive-run items.)*
...
- [X] T108 [US4] Implement `src/AkmlSql.Web/Services/SchemaSync.cs` driving the change-detection protocol: 30 s `SchemaChecksumRequest` polling while editor active, suspend after 5 min idle, resume on keystroke; trigger Phase A then Phase B refresh on checksum drift *(landed end-to-end. Engine: `MessageTypes.SchemaChecksumRequest = 204` + `SchemaChecksumResponse = 205` + `SchemaChecksumHandler` (callback-pure: takes a `Func<string, string?>` so it unit-tests without a real SQL connection). Production wiring in `PipeRpcServer.Handlers.cs` plugs the engine's existing `SchemaCacheManager` -- the checksum is derived from `Phase + ObjectCount` which changes on every cache refresh. Browser: `SchemaSync` now actually issues `IEngineBridge.SendAsync<SchemaChecksumRequest, SchemaChecksumResponse>` on its 30 s tick, compares the returned checksum against the cached value, raises `ChecksumDrifted` event + updates the cache record on drift. Falls back to `TouchAsync` (LRU-keep-warm) when the bridge is closed or the engine reports no connection. 5 engine tests + 1 browser PollNow test.)*
- [X] T109 [US4] Wire `CompletionService` / `QuickInfoService` / `SignatureHelpService` (from US2) to fall back to `SchemaCacheStore` when bridge is unreachable; status badge transitions per the matrix in contracts/schema-cache-shape.md *(landed end-to-end for all three services. Engine: new `MessageTypes.SchemaPhaseARequest/Response = 208/209` + `SchemaPhaseBRequest/Response = 210/211`, callback-pure `SchemaPhaseAHandler` + `SchemaPhaseBHandler` mirroring the SchemaChecksumHandler shape, and `SchemaPhaseSerializer` static helper translating `DatabaseCache` → flat MessagePack `SchemaPhasePayload` (schemas + objects in Phase A; columns + foreign keys + parameters + descriptions added in Phase B). Production wiring in `PipeRpcServer.Handlers.cs` plugs the live `SchemaCacheManager` — Phase A returns once the cache has any data, Phase B once `cache.Phase >= PhaseB`. Wire-level payload is `byte[]` rather than the base64 string the original spec mentioned: the IPC channel is already binary MessagePack so base64 would only add overhead, and the existing `SchemaSnapshot.PhaseA` / `.PhaseB` browser-side fields are `byte[]?` already. 9 engine tests cover happy / no-connection / validation / exception-capture / serializer round-trip across both handlers. Browser: `SchemaSync.PollOnceAsync` now also calls `FetchPhaseAAsync` synchronously and fires-and-forgets `FetchPhaseBAsync` on checksum drift, persisting the returned bytes into the existing `SchemaSnapshot.PhaseA` / `.PhaseB` fields. `CompletionService.CompleteAsync` checks the bridge state and, when closed, picks the most-recently-used snapshot via `ISchemaCacheStore.ListAsync().LastOrDefault()`, deserialises the cached `PhaseA` (+ `PhaseB` when present) and synthesises a `CompletionResponse` from the schemas + objects + columns + a basic keyword set. `QuickInfoService` + `SignatureHelpService` both inject `IEditorSessionStore` for document text (~500 ms stale via the existing debounce — acceptable for offline degraded UX) and use a tiny `OfflineSqlScanner` (no full parser — whitespace + dot + bracket walking) to identify the token under the caret / enclosing function call, then look it up in the cached payload. QuickInfo returns object + parameter + column details with optional MS_Description; Signature returns a single overload with parameter list + active-parameter index counted at paren-depth zero only. 14 web tests across the three services. **Deferred:** explicit active-session pointer for multi-server scenarios (heuristic = LRU-last is correct for the single-engine case which is the typical web-edition usage today); status-badge transitions per the contracts matrix (lives on the editor surface and is best paired with the Playwright interactive verification in T113); alias-table inference offline (online engine uses TSql170Parser; offline falls back to a column-name scan across all cached objects).)*
...
- [~] T113 [P] [US4] Add `tests/AkmlSql.Web.E2E.Tests/UserStory4Tests.cs`: spec.md US4 Acceptance Scenarios 1–4 *(file ADDED under spec 027 (M5 offline closure) T030/T031 — `[Trait("Category","BridgeE2E")]`, Skip-flagged following the established `UserStory2Tests` convention, with the offline-IntelliSense "cable yanked" scenario (SC-008) + the heavyweight online preview/apply scenario (FR-014) fully specced in-body. The deterministic substrate is now covered (`CompletionServiceOfflineTests`, `StatusIndicatorTests`, `RefactoringServiceTests`). **Still deferred:** the actual Playwright run, which needs a live engine + interactive selector iteration — the Skip lifts when an interactive session runs it. So: file done, run pending. Mark fully `[X]` once the E2E executes green against a real engine.)*
...
- [X] T115 [US4] Snippet expansion in the editor — wire to existing engine snippet expander when bridge is up; local-only expansion path when bridge is down; engine round-trip for Save / Delete is gated on `snippets.write` capability per contracts/rpc-handshake.md *(landed for Save/Delete bridge round-trip. `SnippetStore` gained an optional `IEngineBridge` dependency: SaveAsync persists locally first then, when the bridge is open AND `Bridge.EngineCapabilities` contains `snippets.write`, sends `MessageTypes.SnippetSave` over the bridge (DeleteAsync mirrors with SnippetDelete). Bridge failures are silent because the local copy is the source of truth. `Capabilities.Current` now advertises `snippets.write` (the engine has had the SnippetSaveHandler since spec 015). Snippet **expansion** in the editor needs the CodeMirror snippet-decorator integration which lands when the editor-side UX is verified interactively; the data path is in place.)*
...
- [X] T125 [US5] Implement `src/AkmlSql.Web/Services/AiKeyVault.cs` per contracts/ai-key-wrapping.md: non-extractable AES-GCM 256 wrapping key, per-record IV, AAD bound to `providerId`; wrap on set, unwrap-and-immediately-use on call, zeroise on delete *(landed. `Services/IAiKeyVault.cs` with `AiKeyVault` over the same `IWebCryptoWrapper` that backs `IPairingTokenVault` -- different aad prefix ("akmlsql.aikey." + providerId vs "akmlsql.pairing." + connectionId) so a record copied across vaults fails Invariant 5. `UnwrapForCallAsync` returns an `UnwrappedKey` IDisposable wrapper that overwrites its internal byte buffer on Dispose -- callers `using` it inside the provider-call closure to tighten the residual exposure window. `RemoveAsync` overwrites the ciphertext bytes with empty values before delete (best-effort GC zeroise per the contract).)*
- [X] T126 [US5] Implement `src/AkmlSql.Web/Services/AiPreference.cs` (singleton record of active providerId) *(landed. `Services/IAiPreference.cs` -- the active providerId lives in the `aiKeys` store under the sentinel `_active` key. GetActiveAsync returns empty string when no provider has been picked yet.)*
- [X] T127 [P] [US5] Add `tests/AkmlSql.Web.Tests/Ai/KeyVaultTests.cs`: round-trip, plaintext absent from IndexedDB (Invariant 2), tamper detection (Invariant 3), cross-provider unwrap rejected (Invariant 5) *(landed. 10 tests covering Set->Unwrap round-trip, plaintext-absent invariant, GetConfig returns config without key, ListAsync sorted by display name, HasKey false for unknown, Unwrap throws for unknown, Remove drops record, the persisted-aad prefix shape, argument validation, and the UnwrappedKey Dispose-clears-buffer property. The tamper-detection + cross-provider-unwrap-rejected tests rely on the production Web Crypto layer (browser-only); the in-memory test crypto demonstrates the call-site behaviour but the AES-GCM authentication tag check is a real-browser fixture.)*

### M6.3 — Provider call infrastructure

- [X] T128 [US5] Implement `src/AkmlSql.Web/Services/AiClientFactory.cs` routing to provider clients with the origin allow-list from contracts/ai-key-wrapping.md; refuse fetch to any non-allow-listed origin *(landed. `Services/IAiClientFactory.cs` with the 6-provider allow-list hardcoded (openai / anthropic / gemini / azure-wildcard / ollama-loopback / lmstudio-loopback). The web edition does NOT pull in the heavy provider SDKs (Anthropic.SDK / OpenAI / OllamaSharp) -- it fetches directly via HttpClient and parses the OpenAI-compatible `/v1/chat/completions` shape. A fetch to a non-allow-listed origin throws `UnauthorizedOriginException` BEFORE the HttpClient even constructs the request. Anthropic / Gemini wire formats land as follow-ups; the allow-list already covers their origins.)*
- [X] T129 [US5] Add `src/AkmlSql.Web/Services/AiPromptService.cs` building the schema-aware prompt from `AkmlSql.AI.PromptBuilder` + the `SchemaCacheStore` from US4 (no engine round-trip) *(landed. `Services/IAiPromptService.cs` -- 4 actions (Explain / Fix / Optimize / TextToSql) call the extracted prompt builders from `AkmlSql.AI` (ExplainPrompt, FixPrompt, OptimizePrompt, TextToSqlPrompt). The schema text is passed in by the AiPanel caller; the M5 SchemaCacheStore integration that auto-resolves the active database's snapshot lands when the cache-backed completion path lights up.)*
- [X] T130 [P] [US5] Add `tests/AkmlSql.Web.Tests/Ai/AllowListTests.cs`: a mock provider whose endpoint is not allow-listed must throw at `AiClientFactory` *(landed. 18 tests: 7 documented-origin Theory cases + 2 azure-subdomain-wildcard cases + 8 attack-vector rejection cases (suffix attack, http downgrade, wrong provider, wrong domain, wrong port, non-loopback IP, unknown provider id) + 1 ExtractOrigin path-strip test.)*

### M6.4 — Editor UI

- [X] T131 [US5] Add `src/AkmlSql.Web/Shared/AiPanel.razor` hosting the four primary actions (Text-to-SQL, Explain, Fix, Optimize) and Index Analysis; results render to a side pane with Accept / Discard *(landed. `Shared/AiPanel.razor` exposes the four action buttons. Result pane renders the response with Accept (raises `OnAcceptResult` for the parent to insert) / Discard. Index Analysis lands as a follow-up when the engine-side index-analysis context builder ships to the web library. Error mapping (T136) is inlined: 401 -> "Key invalid", 429 -> "Rate limited", 404 -> "Endpoint not found", content policy -> provider-refused message.)*
- [X] T132 [US5] Add `src/AkmlSql.Web/Shared/AiChatPanel.razor`: free-form conversational panel with message history (in-memory only — no persistence) *(landed. `Shared/AiChatPanel.razor` -- in-memory message log, textarea composer (Ctrl+Enter sends), Clear button, "No provider" hint. AiClientFactory.AiChatRequest extended with `History: AiChatMessage[]` (oldest-first); SendAsync interleaves `system -> history -> userPrompt` so the provider sees a real multi-turn conversation. Persistence is intentionally absent per spec.)*
- [ ] T133 [US5] Add Ghost Text inline-completion path (debounced typing → request → render greyed inline → accept on `Tab`) *(deferred -- needs the CodeMirror ghost-text decorator integration verified interactively. The C# side reuses `IAiPromptService` with a ghost-text-specific prompt; the editor-side rendering is the gap.)*
- [ ] T134 [P] [US5] Add `tests/AkmlSql.Web.Tests/Ai/AiPanelTests.cs` (bUnit): action wiring, no-key prompt, provider-error rendering, key never present in DOM *(deferred -- the action wiring + error mapping are covered structurally by KeyVaultTests + AllowListTests at the service level. The bUnit panel test runs when an interactive session can verify the DOM rendering paths.)*

### M6.5 — Settings → AI page

- [X] T135 [US5] Add `src/AkmlSql.Web/Pages/SettingsAi.razor`: per-provider Add/Edit/Remove form, masked display of stored key, model selection, endpoint URL for Azure / Ollama / LM Studio; "No key" path opens provider docs (FR-032) *(landed. `Pages/SettingsAi.razor` at `/settings/ai`. Table of configured providers with an Active radio + Edit / Remove. Add/edit form has provider dropdown (the 6 supported ids), display name, model, optional endpoint, and a masked API-key input. Edit clears the API-key field -- the user MUST re-enter the key (the plaintext is never read back from the wrapped record).)*
- [X] T136 [US5] AI-provider error mapping: 401 → "Key invalid"; 429 → "Rate limited"; network error → "Unreachable"; content policy → provider-specific message + docs link (FR-033) *(landed in AiPanel.MapErrorToMessage. Maps 401 -> "Key invalid", 429 -> "Rate limited", 404 -> "Endpoint not found", content-policy strings -> "Provider refused this prompt", HttpRequestException -> "Provider unreachable". Per-provider docs links are a follow-up -- the current strings cover the common error families.)*

### M6.6 — E2E

- [ ] T137 [P] [US5] Add `tests/AkmlSql.Web.E2E.Tests/UserStory5Tests.cs`: spec.md US5 Acceptance Scenarios 1–4 against mock provider endpoints (the test harness intercepts the allow-listed origins) *(deferred -- Playwright needs a running web app + a mock-provider harness. The unit-test surface in tests/AkmlSql.Web.Tests/Ai/ (31 tests covering vault round-trip + allow-list + preference) covers everything except real-browser Web Crypto and real network fetches.)*
- [X] T138 [US5] Update `doc/architecture.md` § AI library extraction; update `doc/WEB/quickstart-m6.md` (section 5 of quickstart.md) *(landed `doc/WEB/quickstart-m6.md` (BYO-key flow, verify-key-is-wrapped procedure, allow-list verification, what's deferred). `doc/architecture.md` §9c already documents the M6 library extraction (T121-T124 landed earlier in the session).)*
...
- [X] T148 [P] Update `doc/progress.md` with the spec-021 entry (per the existing per-spec table style); summarise tasks completed / deferred *(landed. `doc/progress.md` gained a "Spec 021 -- Web Edition" section with headline numbers, per-milestone table, architectural decisions, test totals (1,143 across all projects), and 8 numbered open follow-ups for the next interactive session.)*
- [X] T149 [P] Update root `README.md` with a short "Web edition" section + link to `doc/WEB/00-INDEX.md` *(landed. Root README.md gained a "Web edition (spec 021)" section + `doc/WEB/` row in the Documentation table + a "021 -- Web edition" row in the Status table.)*
