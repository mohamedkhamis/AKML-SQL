...
- [X] T009 [US1] Expanded `SnippetStore.BuildBuiltIns` from 2 to **11** curated built-ins via a compact declarative list (code-synthesised — the convention `ProfileStore` documents; no EmbeddedResource). Surround-capable entries (`beginend`/`iftest`/`try`) set `SurroundsWith=true`. **CORRECTION mid-task:** the Understand sweep found the engine's `PlaceholderParser` only recognises `$Name$` / `$CURSOR$` / `$SELECTEDTEXT$` — NOT the `${1:label}`/`$selected$` dialect the committed `ssf`/`cte` + the contract used. Per user decision, rewrote ALL built-in bodies to the engine-native syntax so a web-authored `.akmlsnippet` expands correctly on the engine/WPF surface (FR-006/SC-002). **Build+9 SnippetStore tests green.**
- [X] T010 [US1] Added `expandSnippet` + `surroundSelection` exports to `akml-editor.js` + module-level `toCmTemplate`/`toLiteral`/`applySnippetBody` helpers. Translates the engine-native `$Name$`/`$CURSOR$` AND the `SnippetProvider`'s `$1`/`$2` dialect into CM6 `${...}` fields; **feature-detects** CM6 `snippet()` (runtime-unverifiable headless) with a literal-strip fallback in try/catch. Surround substitutes `$SELECTEDTEXT$` then expands. **Build verified; runtime tab-stop behaviour unverified (no browser).**
- [X] T011 [US1] `EditorComponent.ToCmType`: snippet case `"text"`→`"snippet"` so `akml-editor.js`'s `completionSource` routes snippet items to a function `apply` (expand) instead of literal insert; non-snippet items keep the string `apply`. Added `ExpandSnippetAsync`/`SurroundSelectionAsync` C# wrappers. **Build verified.**
- [X] T012 [US1] Added `Ctrl+K, Ctrl+S` to `Editor.razor`'s chord machine → opens a surround-with picker overlay (`.akml-overlay`/`.akml-picker`, `@onclick:stopPropagation`, theme tokens) filtered to `SurroundsWith==true`; pick calls `_editor.SurroundSelectionAsync`. Injected `ISnippetStore`+`IRefactoringService` (latter for US2). **Build verified.**
- [X] T013 [US1] Created `Pages/Snippets.razor` (route `/snippets`, nav link added): two-pane list (built-ins first w/ badge) + editor; create/edit/delete personal via `ISnippetStore` (injected as `SnippetStore` — `Snippets` collides with the page class name); built-in fields disabled + delete hidden. **Build verified.**
- [X] T014 [US1] Import via `<InputFile accept=".akmlsnippet">` (1 MB cap, deserialize→validate shortcode→strip any `builtin.*` id→`SaveAsync`; malformed JSON rejected without corrupting the library); export the selected snippet via `akml-download.js` with a sanitised `<shortcode>.akmlsnippet` filename. **Build verified.**
- [X] T015 [US1] Added `tests/AkmlSql.Web.Tests/Snippets/SnippetBuiltInsTests.cs` (6 tests): built-ins use engine-native syntax (NO `${`/`$selected$` — the cross-surface guard), ssf/cte floor present, surround-capable carry `$SELECTEDTEXT$` (plain ones don't), export→import JSON byte-identical round-trip (FR-006/SC-002), and a builtin-id-import-is-refused-until-cleared guard. **Full `AkmlSql.Web.Tests` = 240 passed, 0 failed.** _JS expansion/surround runtime behaviour (tab-stops landing, selection wrap) is interactively-verified only — not covered headless._
...
- [X] T017 [US2] Added a **Refactor ▾** toolbar button in `Editor.razor` opening a menu overlay (reuses the `.akml-overlay`/`.akml-picker` chrome) listing **all ten** lightweight ops (`LightweightOps` static array, menu order); choosing one calls `Refactoring.PreviewLightweightAsync(kind, _currentText)` (whole-document scope, parser-only, no engine) → shows the preview panel. Menu is never empty offline (FR-010). **Build verified.**
- [X] T018 [US2] Created `src/AkmlSql.Web/Shared/RefactorPreviewPanel.razor` — modal before/after `{before, after, warnings[], changed}` with a defined "no change / not applicable" state and Apply **disabled** when `!Changed` (FR-011); applying calls `ApplyLightweightAsync` then `_editor.SetTextAsync` (one CodeMirror `setText` dispatch = single undoable edit, FR-012); honours the existing 10 MB ceiling via the service path. Added `tests/AkmlSql.Web.Tests/Refactoring/RefactorPreviewPanelTests.cs` (4 bUnit tests: before/after render, no-change+apply-disabled, OnApply raised, OnCancel raised). **Build verified.** _Editor apply round-trip (text actually replaced in CM) is interactive-only; the panel render logic + op output (LightweightParityTests) are test-covered._
...
- [X] T020 [US3] Added a "Schema-aware (requires engine)" section to the refactoring menu in `Editor.razor` with the three PRD heavyweight ops (Smart Rename=0, Parameterize Values=7, Extract Procedure=2). Gated **inline** on `Refactoring.HeavyAvailable` — enabled with a live engine advertising `refactoring.heavy`, else rendered disabled with an "engine" badge + tooltip (gated, never silently absent — FR-017). Inline gating chosen over per-row `<CapabilityNotice>` (cleaner in a list). **Build verified.**
- [X] T021 [US3] Created `RefactorInputDialog.razor` — field-gated (`NeedsOriginalIdentifier`/`NeedsNewName`/`NeedsUnitName`) so Smart Rename collects identifier+new-name, Extract Procedure a proc name, Parameterize Values nothing; client-side required-field validation; raises `RefactorInputs`. Wired in `Editor.razor` → builds `RefactorPreviewRequest` (correct OperationType ints + field names) → `Refactoring.PreviewAsync` (FR-014). **Build verified.**
- [X] T022 [US3] Extended `RefactorPreviewPanel.razor` with a `Heavy` (`RefactorPreviewResponse`) mode: renders `Changes[]` (`-old`/`+new` + loc), `GeneratedObjectTexts[]`, `Warnings[]`, blocking `Errors[]`; Apply gated on `CanApply && Changes.Length>0` (FR-016 — rename collision shows the error, disables Apply). Apply sends `RefactorApplyRequest { OperationType, ApprovedChanges=preview.Changes }`, replaces the editor with the real `RefactorApplyResponse.UpdatedDocumentText` (verified field name — caught a guessed `UpdatedDocuments[]` before build). **Build verified.**
- [X] T023 [P] [US3] Existing 4 gating tests retained; added `RefactorInputDialogTests.cs` (6 bUnit: per-op field gating, empty-field validation blocks submit, valid submit raises trimmed values, heavyweight change-list render+apply-enabled, errors render+apply-disabled). Online bridge round-trip is the US6 E2E path (T031). **Full `AkmlSql.Web.Tests` = 250 passed, 0 failed.** _Online preview/apply against a live engine is interactive/E2E only._
...
- [X] T025 [US4] Added per-finding suppress actions to `ProblemsListComponent.razor` (`⊘line`/`⊘all` buttons, revealed on row hover, with **`@onclick:stopPropagation`** so click-to-jump still works — the sweep's key gotcha). "Suppress on this line" surfaces an `OnSuppressLine` callback → `Editor.razor` inserts ` -- noqa: <RuleId>` at the finding's line end via a new `EditorComponent.InsertAtLineEndAsync` → JS `insertAtLineEnd` (single dispatch = one undo; matches `FixAction.cs`), then re-analyses (FR-018/FR-019). **Build verified.**
- [X] T026 [US4] "Suppress globally" → `Editor.OnSuppressGlobalAsync` writes `RuleOverrides[RuleId]="off"` via `IAnalysisSettingsStore.SetAsync`, then re-analyses; the analyser honours it via the T024 post-pass (FR-020/FR-021). **Made the DI registration explicit** in `Program.cs` (factory injecting `IAnalysisSettingsStore` into `AnalyserService`) so the page and the analyser share the same store instance — closing the T024 "verify DI auto-fill" follow-up. **Build verified.** _The suppression mechanics (noqa parse + override-honoured) are already test-proven by `SuppressionEditTests` (T027); the button wiring is interactive._
...
- [X] T028 [US5] Rewrote `StatusBar.razor` to a cache-aware 4-state (`Live`/`Cached`/`Offline`/`Disconnected` + transient `Connecting`/`Reconnecting`/`Failed`): injects `IConnectionStore`+`ISchemaCacheStore`+`ISchemaSync`, probes the active connection's `(host:port, "master")` snapshot, recomputes on `StateChanged` + `ChecksumDrifted`, holds **Cached** during `Reconnecting`-with-cache (no flicker — flips to Live only on `Open`), preserves the spec-025 reconnect countdown. Best-effort probe (failure ⇒ Offline, never a crash). Added `data-testid="status-pill"` (FR-023/FR-024). **Build verified.**
- [X] T029 [US5] Added `tests/AkmlSql.Web.Tests/Bridge/StatusIndicatorTests.cs` (5 bUnit, reusing `FakeEngineBridge`/`FakeSchemaSync` + real `SchemaCacheStore`/`ConnectionStore`): Open⇒Live (cache irrelevant); Disconnected+cache⇒Cached; Disconnected+no-cache⇒Offline; Reconnecting+cache holds Cached; cache-lands-then-drift flips Offline→Cached in place. **Full `AkmlSql.Web.Tests` = 255 passed, 0 failed** (SC-007 proven).
...
- [~] T030 [US6] Created `tests/AkmlSql.Web.E2E.Tests/UserStory4Tests.cs` — `[Trait("Category","BridgeE2E")]`, **Skip-flagged** following the established `UserStory2Tests` convention (Playwright assertions against an unobserved DOM are high-risk; needs a real engine + interactive selector iteration). The `OfflineIntelliSense_SurvivesEngineKill_FromCache` scenario is fully specced in-body (pair → Live → cache → kill → Cached → completions still resolve (SC-008) → relaunch → Live). **Project builds; the run is developer-side.** Deterministic substrate already covered by `CompletionServiceOfflineTests` + `StatusIndicatorTests`.
- [~] T031 [US6] Added `HeavyweightRename_OnlinePreviewApply_CommitsAcrossSites` to `UserStory4Tests` (same Skip/trait) — the FR-014 online preview/apply scenario (Refactor menu → Smart Rename → input dialog → preview → apply → assert both sites renamed). **Developer-side run.**
- [~] T032 [US6] Both scenarios carry `[Trait("Category","BridgeE2E")]` so the default `dotnet test` excludes them (verified by the existing trait convention); `dotnet test --filter Category=BridgeE2E` runs them **once a maintainer lifts the Skip against a live engine** — cannot execute headless.
- [~] T033 [P] [US6] Authored `specs/027-m5-offline-closure/M5-PARITY-AUDIT.md` (E9) as a structured artifact: the 4-surface table, deltas/closed/accepted-with-reason sections, host-metadata + how-to-run. Marked **PENDING CAPTURE** — the screenshot pass needs a Windows host running both the WPF plugin and the web edition (same constraint as the M2 theme-parity audit), so it's developer-side.
...
- [X] T034 [P] Updated `doc/WEB/quickstart-m5.md`: replaced the stale "What is NOT in M5" caveats with a "Closed by spec 027" section (cache-backed completion, snippets, lightweight + heavyweight refactoring, suppression) and a trimmed "What is *not*" list (engine schema-cache message types, cached-schema heavyweight, file-scope suppression, developer-side E2E/parity).
- [X] T035 [P] Updated spec 021 **T113** in `specs/021-web-edition/tasks.md` to `[~]` (file-added-but-run-deferred) with a note citing spec 027 T030/T031: the `UserStory4Tests.cs` file the task asked for now exists (Skip-flagged, `BridgeE2E`-trait), and the deterministic substrate is covered; the actual Playwright run stays deferred (needs a live engine + interactive selectors). Marked `[~]` rather than `[X]` because the scenarios have not yet *run* green — honest to T113's "Acceptance Scenarios 1–4" wording. Flips to `[X]` when the E2E executes against a real engine.
- [X] T036 [P] Added a spec-027 closure summary to `doc/progress.md` — commit-by-commit table (relocation + US1–US5 with test counts), the two reconciliations, the snippet-syntax correctness catch, and the developer-side/not-runtime-verified list.
- [X] T037 FR-027 DoD walk (M5 PRD §11): `AkmlSql.IntelliSense` exists + both consume it ✓ (shipped); engine suite green after refactor ✓ (98); IndexedDB cache survives restart ✓ (shipped T107); offline IntelliSense cable-yanked ✓ substrate (T109) + E2E scaffold (T030, developer-side run); CHECKSUM_AGG drift ✓ (shipped T108); 9→**10** lightweight refactorings ✓ (US2); 3 heavyweight ✓ as **live-engine** (US3 — cached-schema reconciled to follow-up, Decision 3); snippets built-in+user+import/export+surround ✓ (US1); inline suppression ✓ as **line+global** (US4 — file-scope reconciled to follow-up, Decision 4); parity audit screenshots → **PENDING CAPTURE** (developer-side, M5-PARITY-AUDIT.md); merged-to-master PR → **#245 open**. Every checkbox maps to a shipped feature or FR-001…FR-026; the two reconciled items are scoped-closed. `checklists/requirements.md` unchanged (still all-green; the reconciliations are recorded in spec.md + research.md).
- [X] T038 Final cross-project regression all green: IntelliSense.Tests **8**, engine refactoring gate **98** (relocation non-regressing), `AkmlSql.Web.Tests` **255** (default run), `AkmlSql.Web.E2E.Tests` default **1 passed / 9 skipped** — confirms the `BridgeE2E`-trait Skip-flagged suites (incl. the new `UserStory4Tests`) are correctly excluded from the default run.
