...
- modify, create, or delete any application code — completing the appended tasks is the
  job of `/skill:speckit-implement`.
...
prerequisite command to run (`/skill:speckit-specify` for a missing spec, `/skill:speckit-plan` for a missing plan,
`/skill:speckit-tasks` for missing tasks). Do not produce partial output.
...
- On `tasks_appended`: state how many tasks were appended under which phase, and recommend
  running `/skill:speckit-implement` to complete them; note that a follow-up converge
