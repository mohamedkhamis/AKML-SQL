...
- `checklists/requirements.md` is a separate built-in spec-quality checklist maintained by `/skill:speckit-specify` and `/skill:speckit-clarify`; do not treat that exception as applying to custom checklists generated here.
...
**Important**: Each `/skill:speckit-checklist` command invocation uses a short, descriptive checklist filename and either creates a new file or appends to an existing one. This allows:
