...
namespace AkmlSql.Engine.Server
{
    ...
    /// MessageType registered via <see cref="PipeRpcServer"/>'s pluggable
    /// dictionary implements this interface and returns a fully-formed response
    /// envelope (or <c>null</c> for notifications).
    /// <para>
    /// The hybrid pattern: <see cref="PipeRpcServer.DispatchAsync"/> consults
    ...
    /// changes to <c>PipeRpcServer.cs</c>, satisfying FR-080's invariant
    ...
    internal interface IMessageHandler
    {
        ...
        /// produce no reply (matches <c>PipeRpcServer.DispatchAsync</c>'s own
