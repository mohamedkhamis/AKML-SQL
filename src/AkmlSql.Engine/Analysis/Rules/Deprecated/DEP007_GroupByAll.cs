...
public sealed class DEP007_GroupByAll : IAnalysisRule
{
    ...
    public IEnumerable<AnalysisDiagnostic> Analyze(AnalysisContext ctx)
    {
        ...
        for (var i = 0; i < tokens.Count - 2; i++)
        {
            ...
            if (!tokens[i].Text.Equals("GROUP", System.StringComparison.OrdinalIgnoreCase)) continue;

            // Find the next non-whitespace token
            var j = i + 1;
            while (j < tokens.Count && tokens[j].TokenType == TSqlTokenType.WhiteSpace) j++;
            if (j >= tokens.Count || !tokens[j].Text.Equals("BY", System.StringComparison.OrdinalIgnoreCase)) continue;

            // Find the next non-whitespace token after BY
            var k = j + 1;
            while (k < tokens.Count && tokens[k].TokenType == TSqlTokenType.WhiteSpace) k++;
            if (k >= tokens.Count || !tokens[k].Text.Equals("ALL", System.StringComparison.OrdinalIgnoreCase)) continue;
