...
public class CompletionService : ICompletionService
{
    ...
    private static readonly KeywordInfo[] SqlKeywords = new[]
    {
        ...
        new KeywordInfo("IS DISTINCT FROM", KeywordCategory.Comparison, "NULL-safe not-equal comparison (SQL Server 2022)"),
        new KeywordInfo("IS NOT DISTINCT FROM", KeywordCategory.Comparison, "NULL-safe equal comparison (SQL Server 2022)"),
    ...
    private void AddComparisonOperators(List<CompletionItem> items, string filterText)
    {
        var operators = new[]
        {
            ...
            ("IS DISTINCT FROM", "NULL-safe not-equal comparison (treats NULL as a value)"),
            ("IS NOT DISTINCT FROM", "NULL-safe equal comparison (treats NULL as equal to NULL)"),
            ("LIKE", "Pattern match"),
            ("IN", "In list"),
