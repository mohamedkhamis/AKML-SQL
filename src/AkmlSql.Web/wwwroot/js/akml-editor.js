...
function loadCm() {
    ...
    ]).then(([state, view, commands, langSql, autocomplete, search, lint]) => ({
        state, view, commands, langSql, autocomplete, search, lint,
    }));
...
/** Move the cursor and scroll the given (1-based) line into view. */
export function gotoLine(hostElementId, lineNumber1Based) {
    ...
    inst.view.dispatch({
        ...
        effects: inst.cm.view.EditorView.scrollIntoView(pos, { y: 'center' }),
    });
    inst.view.focus();
}
