...
{
    /// <summary>
    /// Applies casing rules to the active SQL document without changing layout.
    /// </summary>
    internal static class CasingOnlyCommand
    {
        public static void Initialize(AsyncPackage package, OleMenuCommandService commandService)
        {
            var cmdId = new CommandID(PackageGuids.AkmlSqlCmdSet, CommandIds.CmdCasingOnly);
        ...
        private static void Execute(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Log.Warning("CasingOnly: engine not available");
                    return;
                }
                ...
                System.Threading.Tasks.Task.Run(async () =>
                {
                    ...
                    {
                        Log.Error(ex, "CasingOnly via engine failed");
                    }
                });
            }
            catch (Exception ex)
            {
                Log.Error(ex, "Casing only command failed");
            }
        }
    }
}
