...
{
    /// <summary>
    /// Expands UPDATE SET clauses to list all table columns explicitly.
    /// </summary>
    internal static class ExpandUpdateColumnsCommand
    {
        public static void Initialize(AsyncPackage package, OleMenuCommandService commandService)
        {
            var cmdId = new CommandID(PackageGuids.AkmlSqlCmdSet, CommandIds.CmdExpandUpdateColumns);
        ...
        private static void Execute(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var request = new FormatActionRequest
                {
                    ...
                    Text            = documentText,
                    ActionType      = (int)FormatActionType.ExpandUpdateColumns,
                    ...
                    try
                    {
                        ...
                        if (response.Warnings != null && response.Warnings.Length > 0)
                        {
                            ...
                            VsShellUtilities.ShowMessageBox(
                                null,
                                string.Join("\n", response.Warnings),
                                "AKML SQL \u2013 Expand UPDATE Columns",
                                OLEMSGICON.OLEMSGICON_INFO,
                                OLEMSGBUTTON.OLEMSGBUTTON_OK,
                                OLEMSGDEFBUTTON.OLEMSGDEFBUTTON_FIRST);
                        }
                    }
                    catch (Exception ex) { Log.Error(ex, "ExpandUpdateColumns failed"); }
                });
            }
            catch (Exception ex) { Log.Error(ex, "ExpandUpdateColumns command failed"); }
        }
    }
}
