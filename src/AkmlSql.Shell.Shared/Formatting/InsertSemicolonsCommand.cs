...
{
    /// <summary>
    /// Inserts semicolons after each statement in the active SQL document.
    /// </summary>
    internal static class InsertSemicolonsCommand
    {
        public static void Initialize(AsyncPackage package, OleMenuCommandService commandService)
        {
            var cmdId = new CommandID(PackageGuids.AkmlSqlCmdSet, CommandIds.CmdInsertSemicolons);
        ...
        private static void Execute(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Log.Warning("InsertSemicolons: engine not available");
                    return;
                }
                ...
                System.Threading.Tasks.Task.Run(async () =>
                {
                    ...
                    {
                        Log.Error(ex, "InsertSemicolons via engine failed");
                    }
                });
            }
            catch (Exception ex)
            {
                Log.Error(ex, "Insert semicolons command failed");
            }
        }
    }
}
