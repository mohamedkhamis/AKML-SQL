...
{
    /// <summary>
    /// Converts comma-separated (old-style) implicit joins to explicit ANSI JOIN syntax.
    /// Non-equi JOIN conditions are left in the WHERE clause and surfaced as warnings.
    /// </summary>
    internal static class ConvertOldStyleJoinsCommand
    {
        public static void Initialize(AsyncPackage package, OleMenuCommandService commandService)
        {
            var cmdId = new CommandID(PackageGuids.AkmlSqlCmdSet, CommandIds.CmdConvertOldStyleJoins);
        ...
        private static void Execute(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var request = new FormatActionRequest
                {
                    ...
                    Text            = documentText,
                    ActionType      = (int)FormatActionType.ConvertOldStyleJoins,
                    ...
                    try
                    {
                        ...
                        if (response.Warnings != null && response.Warnings.Length > 0)
                        {
                            ...
                            VsShellUtilities.ShowMessageBox(
                                null,
                                string.Join("\n", response.Warnings),
                                "AKML SQL \u2013 Convert Old-Style JOINs",
                                OLEMSGICON.OLEMSGICON_INFO,
                                OLEMSGBUTTON.OLEMSGBUTTON_OK,
                                OLEMSGDEFBUTTON.OLEMSGDEFBUTTON_FIRST);
                        }
                    }
                    catch (Exception ex) { Log.Error(ex, "ConvertOldStyleJoins failed"); }
                });
            }
            catch (Exception ex) { Log.Error(ex, "ConvertOldStyleJoins command failed"); }
        }
    }
}
