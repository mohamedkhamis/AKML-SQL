...
{
    /// <summary>
    /// Appends a GROUP BY clause populated from non-aggregated SELECT expressions.
    /// </summary>
    internal static class AddGroupByColumnsCommand
    {
        public static void Initialize(AsyncPackage package, OleMenuCommandService commandService)
        {
            var cmdId = new CommandID(PackageGuids.AkmlSqlCmdSet, CommandIds.CmdAddGroupByColumns);
        ...
        private static void Execute(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var request = new FormatActionRequest
                {
                    ...
                    Text            = documentText,
                    ActionType      = (int)FormatActionType.AddGroupByColumns,
                    ...
                    try
                    {
                        ...
                        if (response.Warnings != null && response.Warnings.Length > 0)
                        {
                            ...
                            VsShellUtilities.ShowMessageBox(
                                null,
                                string.Join("\n", response.Warnings),
                                "AKML SQL \u2013 Add GROUP BY Columns",
                                OLEMSGICON.OLEMSGICON_INFO,
                                OLEMSGBUTTON.OLEMSGBUTTON_OK,
                                OLEMSGDEFBUTTON.OLEMSGDEFBUTTON_FIRST);
                        }
                    }
                    catch (Exception ex) { Log.Error(ex, "AddGroupByColumns failed"); }
                });
            }
            catch (Exception ex) { Log.Error(ex, "AddGroupByColumns command failed"); }
        }
    }
}
