...
{
    /// <summary>
    /// Rewrites EXEC statements with positional parameters to use named parameter form.
    /// </summary>
    internal static class ExpandExecParametersCommand
    {
        public static void Initialize(AsyncPackage package, OleMenuCommandService commandService)
        {
            var cmdId = new CommandID(PackageGuids.AkmlSqlCmdSet, CommandIds.CmdExpandExecParameters);
        ...
        private static void Execute(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var request = new FormatActionRequest
                {
                    ...
                    Text            = documentText,
                    ActionType      = (int)FormatActionType.ExpandExecParameters,
                    ...
                    try
                    {
                        ...
                        if (response.Warnings != null && response.Warnings.Length > 0)
                        {
                            ...
                            VsShellUtilities.ShowMessageBox(
                                null,
                                string.Join("\n", response.Warnings),
                                "AKML SQL \u2013 Expand EXEC Parameters",
                                OLEMSGICON.OLEMSGICON_INFO,
                                OLEMSGBUTTON.OLEMSGBUTTON_OK,
                                OLEMSGDEFBUTTON.OLEMSGDEFBUTTON_FIRST);
                        }
                    }
                    catch (Exception ex) { Log.Error(ex, "ExpandExecParameters failed"); }
                });
            }
            catch (Exception ex) { Log.Error(ex, "ExpandExecParameters command failed"); }
        }
    }
}
