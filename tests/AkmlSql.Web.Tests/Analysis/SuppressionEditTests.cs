...
///   * Line  — the browser emits the shared <c>-- noqa: RULEID</c> directive; this test
///     proves that exact string is honoured by the real <c>AkmlSql.Analysis.SuppressionParser</c>
///     (cross-surface format contract, FR-019/FR-022).
...
{
    // ---- Line scope: the -- noqa: RULEID directive is the shared format ----

    [Fact]
    public void Line_directive_in_shared_noqa_form_is_parsed_by_the_engine_parser()
    {
        // The exact string the browser appends at the finding's line end.
        const string ruleId = "PE001";
        var line = $"SELECT * FROM dbo.Orders; -- noqa: {ruleId}";
