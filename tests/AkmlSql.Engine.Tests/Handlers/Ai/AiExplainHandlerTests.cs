...
// Spec 022 (M0 closure) -- P3 / US3 follow-up. Smoke tests for AiExplainHandler.
...
public class AiExplainHandlerTests
{
    ...
    private static AiExplainHandler NewHandler(AiSettings settings) =>
    ...
    public void Message_type_ids_match_the_wire_contract()
    {
        ...
        Assert.Equal(MessageTypes.AiExplain, handler.RequestMessageType);
        Assert.Equal(MessageTypes.AiExplainResult, handler.ResponseMessageType);
    }
    ...
    public async Task Returns_error_response_when_AI_assistance_disabled()
    {
        ...
        var resp = await handler.HandleAsync(
            new AiExplainRequest { SessionId = "test", SelectedSql = "SELECT 1" },
            NewContext(),
    ...
    public async Task Returns_error_response_when_sql_is_empty()
    {
        var handler = NewHandler(new AiSettings { Provider = "ollama", Enabled = true });
        var resp = await handler.HandleAsync(
            new AiExplainRequest { SessionId = "test", SelectedSql = "   " },
            NewContext(),
            CancellationToken.None);
