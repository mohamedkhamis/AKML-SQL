...
// Spec 022 (M0 closure) -- P3 / US3 follow-up. Smoke tests for AiFixHandler.
...
public class AiFixHandlerTests
{
    ...
    private static AiFixHandler NewHandler(AiSettings settings) =>
    ...
    public void Message_type_ids_match_the_wire_contract()
    {
        ...
        Assert.Equal(MessageTypes.AiFix, handler.RequestMessageType);
        Assert.Equal(MessageTypes.AiFixResult, handler.ResponseMessageType);
    }
    ...
    public async Task Returns_error_response_when_AI_assistance_disabled()
    {
        ...
        var resp = await handler.HandleAsync(
            new AiFixRequest { SessionId = "test", FailingSql = "SELECT 1" },
            NewContext(),
    ...
    [Fact]
    public async Task Returns_error_response_when_failing_sql_is_empty()
    {
        var handler = NewHandler(new AiSettings { Provider = "ollama", Enabled = true });
        var resp = await handler.HandleAsync(
            new AiFixRequest { SessionId = "test", FailingSql = "   " },
            NewContext(),
            CancellationToken.None);
