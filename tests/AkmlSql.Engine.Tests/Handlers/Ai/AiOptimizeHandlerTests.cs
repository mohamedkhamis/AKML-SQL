...
// Spec 022 (M0 closure) -- P3 / US3 follow-up. Smoke tests for AiOptimizeHandler.
...
public class AiOptimizeHandlerTests
{
    ...
    private static AiOptimizeHandler NewHandler(AiSettings settings) =>
    ...
    public void Message_type_ids_match_the_wire_contract()
    {
        ...
        Assert.Equal(MessageTypes.AiOptimize, handler.RequestMessageType);
        Assert.Equal(MessageTypes.AiOptimizeResult, handler.ResponseMessageType);
    }
    ...
    public async Task Returns_error_response_when_AI_assistance_disabled()
    {
        ...
        var resp = await handler.HandleAsync(
            new AiOptimizeRequest { SessionId = "test", SelectedSql = "SELECT 1" },
            NewContext(),
    ...
    public async Task Returns_error_response_when_sql_is_empty()
    {
        var handler = NewHandler(new AiSettings { Provider = "ollama", Enabled = true });
        var resp = await handler.HandleAsync(
            new AiOptimizeRequest { SessionId = "test", SelectedSql = "   " },
            NewContext(),
            CancellationToken.None);
