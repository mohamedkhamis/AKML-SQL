...
|--------|------------|---------------|
| Anyone on the LAN connects to the WebSocket | Pairing-token bearer auth required. The engine's `HandshakeHandler` enforces this when `RequirePairingToken` is set on the transport options. | None for a single-engine deployment; multi-engine sharing of `tokens.json` is out of scope. |
...
| Brute force the 6-digit PIN | PIN is single-use; expires after 5 min; after 3 wrong attempts, engine refuses pairing for 15 min (`PairingService.ValidatePin`'s sliding-window rate limiter). | A determined attacker has ~3 attempts per 15-min window over a 5-min validity → vanishingly small success probability. |
